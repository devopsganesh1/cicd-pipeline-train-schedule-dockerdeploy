pipeline {
    ...
    stages {
        ...
        stage('Build Docker Image') {
            ...
            steps {
                script {
                    app = docker.build("dockerganesh/train-schedule")
                    app.inside {
            ...
            steps {
                ...
                withCredentials([usernamePassword(credentialsId: 'dockerslave', usernameVariable: 'USERNAME', passwordVariable: 'USERPASS')]) {
                    script {
                        ...
                        sh "sshpass -p '$USERPASS' -v ssh -o StrictHostKeyChecking=no $USERNAME@$prod_ip \"docker run --restart always --name train-schedule -p 8080:8080 -d dockerganesh/train-schedule:${env.BUILD_NUMBER}\""
                    }
                }
            }
        }
